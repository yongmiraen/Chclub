...
export default async function Home({
  ...
  const sort: SortKey =
    sp.sort === "popular" || sp.sort === "new" ? sp.sort : "discover";
          ...
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                {t.label}
                {active && (
                  <span className="absolute inset-x-6 -bottom-px h-0.5 bg-stone-900" />
                )}
              </Link>
            );
          })}
      ...
      <section>
        ...
        {error && (
          <div className="mx-4 rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">
            데이터를 불러오지 못했습니다. .env.local 과 sql/schema.sql 적용을
            확인해 주세요.
            <br />
