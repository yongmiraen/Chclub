...
function Section({ title, group }: { title: string; group: "신앙" | "취미" }) {
  ...
  return (
    <section>
      <h2 className="mb-2 text-xs font-medium tracking-wide text-stone-500">{title}</h2>
      <ul className="grid grid-cols-2 overflow-hidden rounded-2xl border border-stone-200 bg-white">
        {items.map((c, i) => {
          ...
          return (
            <li key={c.slug} className={`${right ? "border-r border-stone-100" : ""} ${bottom ? "border-b border-stone-100" : ""}`}>
              <Link href={`/categories/${c.slug}`}
                className="flex items-center gap-3 px-4 py-3.5 active:bg-stone-50">
                <span className={`grid h-9 w-9 place-items-center rounded-full ${tone.bg} text-lg ring-1 ${tone.ring}`}>
                  {c.emoji}
                </span>
